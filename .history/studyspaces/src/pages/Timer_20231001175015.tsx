...
const TimerDialog = () => {
  ...
  useEffect(() => {
    ...
    if (isRunning && timeRemaining > 0) {
      timerId = setInterval(() => {
        setTimeRemaining((prevTime) => {
          ...
          // If the timer reaches 0, stop it and play the notification sound
          if (newTime === 0) {
            setIsRunning(false);
    ...
    // Save customTime to localStorage whenever it changes
    localStorage.setItem('customTime', customTime);

    // Cleanup: Clear the interval when the component unmounts or when isRunning changes
    return () => clearInterval(timerId);
  }, [isRunning, timeRemaining, customTime]);
