...
const TimerDialog = () => {
  const [timeRemaining, setTimeRemaining] = useState(parseInt(customTime)); 
  ...
  const startTimer = () => {
    const storedTimeRemaining = localStorage.getItem('timeRemaining');
    const timeInSeconds = storedTimeRemaining ? parseInt(storedTimeRemaining) : parseInt(customTime) * 60;
  ...
  const resetTimer = () => {
    setTimeRemaining(parseInt(customTime));
    setIsRunning(false);
    setCustomTime(''); 
  };
