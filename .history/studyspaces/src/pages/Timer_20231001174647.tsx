...
const TimerDialog = ({
  ...
  useEffect(() => {
    ...
    if (isRunning && timeRemaining > 0) {
      timerId = setInterval(() => {
        // Update the timeRemaining prop instead of local state
        // Also, ensure you're using a function to update it
        setCustomTime((prevTime) => (parseInt(prevTime) - 1).toString());
      }, 1000);
    }

    if (timeRemaining === 0) {
      clearInterval(timerId);
      stopTimer(); // Call the stopTimer function from props
