...
// manages the running of the timer
  ...
  const startTimer = () => {
    if (!isRunning && !wasPaused) {
      ...
      setTimeRemaining(timeInSeconds);   // `setTimeRemaining` function is used to update the `timeRemaining` state variable with the calculated value
      setIsRunning(true);   // `setIsRunning` function is called to set the `isRunning` state variable to `true`
      localStorage.setItem('customTime', customTime);   // `localStorage.setItem` method is used to store the `customTime` value in the browser's local storage
    } else {
